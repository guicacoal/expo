...
import { createStableModuleIdFactory, getDefaultConfig, type LoadOptions } from '@expo/metro-config';
...
interface MetroServerWithModuleIdMod extends MetroServer {
  _createModuleId: |
    ReturnType<typeof createStableModuleIdFactory> & ((path: string) => number);
}
...
export async function instantiateMetroAsync(
  ...
  if (!isExporting) {
    ...
    asWritable(metroConfig.server).enhanceMiddleware = (metroMiddleware: any, server: MetroServer) => {
